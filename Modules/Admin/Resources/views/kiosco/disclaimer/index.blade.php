...
<div class="row textbox">
    ...
    <div class="col-sm-6 text-right">
        @if (Auth::action('legals.create'))
            <a href="{{ route('admin.kiosco.disclaimer.create') }}" class="btn btn-warning addButton">
                    ...
                    <div class="row">
                        <div class="col-xs-4 text-center">
                            @if($leg->active == 1)
                                @if (Auth::action('legals.off'))
                                    <form name="formOff_{{ $leg->id }}" method="POST" action="{{ route('admin.kiosco.disclaimer.changeStatus') }}">
                                        {{ csrf_field() }}
                            ...
                            @else
                                @if (Auth::action('legals.on'))
                                    <form name="formOn_{{ $leg->id }}" method="POST" action="{{ route('admin.kiosco.disclaimer.changeStatus') }}">
                                        {{ csrf_field() }}
                        ...
                        <div class="col-xs-4 text-center">
                            @if (Auth::action('legals.edit'))
                                <a class="fa fa-pencil itemTooltip" href="{{ route('admin.kiosco.disclaimer.edit', ['id' => $leg->id]) }}"
                               title="{{ trans('admin::shopping.legals.index.edit') }}" style="color: black"></a>
                            @endif
                        </div>
                        <div class="col-xs-4 text-center">
                            @if (Auth::action('legals.delete'))
                                <form name="formDel_{{ $leg->id }}" method="POST" action="{{ route('admin.kiosco.disclaimer.destroy', ['id' => $leg->id]) }}">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <input type="hidden" name="id" value="{{ $leg->id }}"/>
