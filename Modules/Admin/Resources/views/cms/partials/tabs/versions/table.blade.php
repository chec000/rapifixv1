    ...
    <table class="table table-striped">
        ...
        @foreach ($versions as $version)
            <tr id="v_{{ $version->version_id }}">
                ...
                <td>
                    <a href="{{ Modules\CMS\Helpers\Page\Path::getFullUrl($version->page_id).'?preview='.$version->preview_key }}"
                       target="_blank"><i class="glyphicon glyphicon-eye-open itemTooltip" title="Preview"></i></a>
                    <a href="{{ route('admin.pages.edit', ['pageId' => $version->page_id, 'version' => $version->version_id]) }}"><i
