<h1> {{trans('admin::shopping.securityquestions.update.updateoe')}}</h1>
<p class="text-right"><a href="{{ route('admin.securityquestions.list') }}">{{trans('admin::shopping.securityquestions.add.back_list')}}</a></p>
{!! Form::open( ['id'=>'form_update_securityquestions']) !!}
<input type="hidden" value="{!!$securityquestions->id!!}" name="id_security_questions">
<div class="row">

    <fieldset class="fieldset_gray">
        <legend class="legend_gray">{!! trans('admin::shopping.securityquestions.add.legend_add') !!}</legend>
        <div class="col-md-6">
            <div  class= "form-group {!! FormMessage::getErrorClass('key') !!}">
                {!! Form::label('key', trans('admin::shopping.securityquestions.add.key'), ['class' => 'control-label']) !!}

                {!! Form::text('key', isset($securityquestions->key_question) ? $securityquestions->key_question:'', ['required','class' => 'form-control','id'=>'key']) !!}
        ...
        <div class="col-md-6">
            <div class="form-group   {!! FormMessage::getErrorClass('country_id') !!}">
                {!! Form::label('country_id', trans('admin::shopping.securityquestions.add.countries'), ['class' => 'control-label']) !!}
...
@if(isset($update))
<h3>{{ trans('admin::shopping.securityquestions.add.oe_translates') }}</h3>
<p class="text-danger" style="font-style: italic;">{{ trans('admin::shopping.securityquestions.add.oe_disclaimer') }}</p>
@if($errors->any())
            ...
            <div class="panel-body">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-sm-6">
                        <label for="language_lang" class="control-label">{{ trans('admin::shopping.securityquestions.add.name') }}</label>
                        <input  class="form-control" name="securityquestions_name[{{$lan->locale_key}}]" value="{{$lan->name or old('securityquestions_name.'.$lan->locale_key)}}" type="text">
                        <input  class="form-control" name="securityquestions_locale[]" value="{!!$lan->locale_key !!}" type="hidden">
                    </div>
...
<div class="form-group">
    ...
    {!!Form::submit(trans('admin::shopping.securityquestions.update.btn_update'),[ 'class'=>'btn btn-primary addButton'])!!}
