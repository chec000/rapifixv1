<?php namespace Modules\Admin\Helpers\Import;
use Modules\CMS\Entities\Block as b;
class BlocksCollection
{
    ...
    public function getBlocks($scope = '')
    {
        ...
        public function getBlockName($name,$category){
        try {
            $block= b::where([['name','=',$name],['category_id','=',$category],['active','=',1]])->first();        
            if($block!=null){
                       
                $block_info=  $block->attributesToArray();                   
                       if (array_key_exists("label", $block_info)) {
                           return $block_info['label'];
                       }else{
                           return "";
                       }
