...
use Auth;
use Cache;
use Modules\Admin\Helpers\Validation\Database;
...
class SystemController extends Controller
{
    ...
    public function getIndex()
    {
        ...
        $upgrade->required = version_compare(config('cms.site.version'), Setting::latestTag(), '<');

        $this->layoutData['content'] = View::make('admin::cms.pages.system', array('database_structure' => $database_structure, 'last_indexed_search' => $last_indexed_search, 'site_details' => $settings, 'can_index_search' => Auth::action('system.search'), 'can_validate' => Auth::action('system.validate-db'), 'can_upgrade' => Auth::action('system.upgrade'), 'upgrade' => $upgrade));
    }

    public function getWpLogin()
    {
        $blog_login_url = URL::to('/' . trim(config('cms.blog.url'), '/') . '/wp-login.php');
        $blog_admin_url = URL::to('/' . trim(config('cms.blog.url'), '/') . '/wp-admin/');
    ...
    public function getValidateDb($fix = null)
    {
        ...
        $this->layoutData['content'] = View::make('admin::cms.pages.system.validate-db', $messages);
        return null;
    }
    ...
    public function getUpgrade($update = null)
    {
        ...
        if (empty($update)) {
        ...
        } elseif (version_compare(config('cms.site.version'), Setting::latestTag(), '<') && Setting::latestTag() != "not-found") {

            if (!getenv('HOME') && !getenv('COMPOSER_HOME')) {
                putenv("COMPOSER_HOME=".exec('pwd')."/.composer");
            }

            $coasterStorage = storage_path(config('cms.site.storage_path'));
            ...
            if (is_writable($logFile)) {
                ...
                if (!empty($upgradeLog) && stripos($upgradeLog, 'Generating autoload files') !== false) {
                    Cache::put('admin::site.version', Setting::latestTag(), 30);
            ...
            $message = 'Already at the latest version '.config('cms.site.version');
        }

        $this->layoutData['content'] = View::make('admin::cms.pages.system.upgrade', ['message' => $message, 'error' => $error, 'run' => $run]);
    ...
    private function _db_messages($basic_fix = 0)
    {
        ...
        if ($basic_fix == 1) {
            return ['redirect' => \redirect()->route('admin.system.validate-db')];
        }
