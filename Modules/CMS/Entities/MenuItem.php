...
use Modules\CMS\Libraries\Traits\Logger;
use Eloquent;
use View;
use Request;
...
{
    use Logger, DataPreLoad, Translatable;

    public $translatedAttributes = ['custom_name'];
    protected $table = 'cms_menu_items';

    public function page()
    {
        return $this->belongsTo('Modules\CMS\Entities\Page');
    }
        ...
        if ($subLevels > 0 && $childPages = Page::getChildPages($parentPageId)) {
            ...
            foreach ($childPages as $childPage) {
                if ($childPage->is_live()) {
                    ...
                    $renderedPages .= View::make('admin::cms.partials.menus.page_li', ['page' => $childPage, 'item' => $this, 'name' => $name, 'leaf' => $leaf, 'permissions' => $permissions])->render();
                }
            }
            if ($renderedPages) {
                return View::make('admin::cms.partials.menus.page_ol', ['renderedPages' => $renderedPages, 'item' => $this, 'depth' => $depth])->render();
            }
        }
        return '';
    }
    ...
    public function getCustomNameV2($pageId = 0)
    {
        if ($pageId) {
            $pageNames = @unserialize($this->custom_page_names);
            if (is_array($pageNames) && array_key_exists($pageId, $pageNames)) {
            ...
            if(!empty(Request::get('language_id'))) {
                ...
                $menuItemTranslation = MenuItemTranslation::getMenuItemTranslation($this->id, $language->locale_key);
                //dd($menuItemTranslation);
                return $menuItemTranslation['custom_name'];
            } else {
                return '';
            }
