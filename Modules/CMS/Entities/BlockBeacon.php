...
use Modules\CMS\Libraries\Beacons\BeaconAbstract;
...
{
    protected $table = 'cms_block_beacons';
    ...
    public static function getClass($type = 'Kontakt')
    {
        ...
        if (!array_key_exists($type, static::$_class)) {
            $className = 'Modules\CMS\Libraries\Beacons\\'.$type.'Beacon';
    ...
    public static function preload($addMissing = false)
    {
        if (!isset(self::$_beacons)) {
            ...
            try {
                $enabledDrivers = [
                    'Kontakt' => config('cms.key.kontakt'),
                    'Estimote' => config('cms.key.estimote')
                ];
    ...
    public static function getTableRows()
    {
        if ($beaconsData = self::getVisibleBeacons()) {
            return View::make('admin::cms.partials.themes.beacons.rows', ['beaconsData' => $beaconsData]);
        } else {
            return '';
        }
    }
            ...
            if ($pageId) {
                ...
                try {
                    $bitlyResponse = json_decode(self::_bitly()->request('GET', 'v3/shorten', [
                        'query' => [
                            'access_token' => config('cms.key.bitly'),
