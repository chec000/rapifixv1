...
use Modules\CMS\Events\Page\Search;
use Modules\CMS\Helpers\Page\Search\Cms;
use Modules\CMS\Helpers\Page\Search\WordPress;
use Eloquent;

class PageSearchData extends Eloquent
{

    protected $table = 'cms_page_search_data';

    public function block()
    {
        return $this->belongsTo('Modules\CMS\Entities\Block');
    }

    public static function updateText($content, $blockId, $pageId, $languageId = null)
    {
        $languageId = $languageId ?: \Modules\Admin\Entities\Language::current();
        ...
        if ($content) {
            ...
            $searchData->language_id = $languageId;
    ...
    public static function lookup($keywords, $onlyLive = 1, $limit = 100)
    {
        ...
        if (!empty($keywords)) {
            ...
            $searchObjects = [
                new Cms($onlyLive),
                new WordPress($onlyLive, config('cms.blog.url') ? Setting::blogConnection() : false)
            ];
