...
class Block extends Eloquent
{
    ...
    {
        return $this->hasMany('Modules\CMS\Entities\PageBlockDefault');
    }
    ...
    public function pageCustomLang()
    {
        ...
        return $this->hasOne('Modules\CMS\Entities\BlockTranslation')->where('locale', '=', !empty(Request::get('language_id')) ? $language->locale_key : 0);
    }
    /**
    ...
    public static function getBlockOnPage($blockId, $pageId)
    {
        if ($page = Page::find($pageId)) {
            $blocksByCat = ThemeTemplate::templateBlocks(config('cms.frontend.theme'), $page->template);
        } else {
            $blocksByCat = Theme::theme_blocks(config('cms.frontend.theme'));
        }
    ...
    /**
     * @return \Modules\CMS\Libraries\Blocks\String_
     */
    ...
    protected static function _loadClasses($reload = false)
    {
        if (!static::_preloadIsset('blockClass') || $reload) {
            $paths = [
                'Modules\\CMS\\Libraries\\Blocks\\' => base_path('Modules/CMS/Libraries/Blocks'),
    ...
    public static function getDataForVersion($model, $version, $filters = [], $orderBys = [])
    {
        ...
        switch ($tableName) {
            case 'cms_page_blocks': $identifiers = ['block_id', 'language_id', 'page_id']; break;
            case 'cms_page_blocks_default': $identifiers = ['block_id', 'language_id', 'brand_id', 'country_id']; break;
            case 'cms_page_blocks_repeater_data': $identifiers = ['block_id', 'row_key']; break;
     ...
     * @return array
     */
    public static function getTabs($categorisedBlocks, $blockContents, $pageId = 0, $versionId = 0, $lang = 0)
    {
            $current_lang = ($lang != 0) ?$lang:   \Modules\Admin\Entities\Language::current();
        ...
        foreach ($categorisedBlocks as $categoryId => $categoryBlocks) {
          ...
          foreach ($categoryBlocks as $blockId => $block) {
                ...
                $blockContent = isset($blockContents[$blockId][$current_lang]) ? $blockContents[$blockId][$current_lang]->content : '';
                switch ($block->type){
                    case 'link':
                            $tabContents[$tabIndex] .= $block->setPageId($pageId)->setVersionId($versionId)->getTypeObject()->edit($blockContent,$pageId,$current_lang);
                        break;
                    case 'repeater':
                            $tabContents[$tabIndex] .= $block->setPageId($pageId)->setVersionId($versionId)->getTypeObject()->edit($blockContent,false,$current_lang);
                        break;
                    default :
