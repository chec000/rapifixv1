...
class PageBlockRepeaterData extends Eloquent
{
    ...
    protected $table = 'cms_page_blocks_repeater_data';
    ...
    public static function getBlockData($blockId, $versionId, $repeaterId, $repeaterRowId)
    {
        ...
        $getDataQuery = static::where('block_id', '=', $blockId)->where('row_key', '=', $repeaterRowKey)->where('language_id', '=', \Modules\Admin\Entities\Language::current());
        if (!empty($versionId)) {
    ...
    public static function loadRepeaterData($repeaterId, $version = 0, $randomOrder = false, $search = '')
    {
        $repeaterRowsById = [];
        if (!empty($search)) {
            $rowIds = PageBlockRepeaterRows::getRowIdsInSearch($repeaterId, $search);
        } else {
            $rowIds = PageBlockRepeaterRows::getRowIds($repeaterId);
        }
