...
use Modules\CMS\Helpers\File\Directory;
use Modules\CMS\Helpers\File\SecureUpload;
use Modules\CMS\Helpers\File\Zip;
use Modules\CMS\Helpers\Page\PageCache;
use Modules\CMS\Libraries\Export\BlocksExport;
use Modules\CMS\Libraries\Export\ContentExport;
use Modules\CMS\Libraries\Export\GroupsExport;
use Modules\CMS\Libraries\Export\MenusExport;
use Modules\CMS\Libraries\Export\PagesExport;
use Modules\CMS\Libraries\Import\BlocksImport;
use Modules\CMS\Libraries\Import\ContentImport;
use Modules\CMS\Libraries\Import\GroupsImport;
use Modules\CMS\Libraries\Import\MenusImport;
use Modules\CMS\Libraries\Import\PagesImport;
use DB;
use Eloquent;
...
Class Theme extends Eloquent
{
    ...
    public function templates()
    {
        ...
        $t1 = $this->belongsToMany('Modules\CMS\Entities\Template', $themeTemplatesTableNoPrefix)
        ...
        return Template::join($t2, $themeTemplatesTableNoPrefix.'.id', '=', $templatesTableNoPrefix.'.id')
            ...
            //->addSelect($templatesTableNoPrefix . '.label')
    ...
    {
        return $this->templates()->where('cms_templates.id', '=', $templateId)->first();
    }

    public function blocks()
    {
        return $this->belongsToMany('Modules\CMS\Entities\Block', 'cms_theme_blocks')->withPivot('show_in_pages', 'exclude_templates', 'show_in_global')->where('active', '=', 1)->orderBy('order', 'asc');
    }

    public function blocksGlobal()
    {
        return $this->belongsToMany('Modules\CMS\Entities\Block', 'cms_theme_blocks')->withPivot('show_in_pages', 'exclude_templates', 'show_in_global')->where('active', '=', 1)->wherePivot('show_in_global', '=', 1)->orderBy('order', 'asc');
    }

    public static function get_template_list($includeTemplate = 0)
    {
        $templates = [];
        if ($theme = static::find(config('cms.frontend.theme'))) {
            foreach ($theme->templates()->where('cms_theme_templates.hidden', '=', 0)->get() as $template) {
    ...
    public static function theme_blocks($theme, $template = 0)
    {
        ...
        if (!empty($selected_theme)) {
            ...
            foreach ($theme_blocks as $theme_block) {
                ...
                if ($theme_block->active == 1 && ((!$in_pages && $theme_block->pivot->show_in_global == 1) || ($in_pages && $theme_block->pivot->show_in_pages == 1))) {
        ...
        if (!$templates->isEmpty() && $themeId) {
            ...
            if (!empty($themeTemplatesById)) {
                ...
                if (empty($defaultTemplate)) {
                    ...
                    $defaultTemplateId = config('admin.config.default_template');
                }
    ...
    public static function remove($themeName)
    {
        $theme = self::where('theme', '=', $themeName)->first();
        if (!empty($theme)) {
            if ($theme->id == config('cms.frontend.theme')) {
                return 0;
            }
