...
abstract class AbstractBlock
{
    ...
    protected function _renderRepeatedDisplayView($options, $data = [])
    {
        if ($displayView = $this->displayViewOptions($options)) {
            ...
            if (!empty($data) && is_array($data)) {
                ...
                $gradientTheme = !empty($options['gradient_theme']) ? $options['gradient_theme'] : '';
                $signalTitle = !empty($options['signal_title']) ? $options['signal_title'] : '';
                $signalTitleHighlight = !empty($options['signal_title_highlight']) ? $options['signal_title_highlight'] : '';
                ...
                foreach ($data as $dataItemId => $dataItem) {
                    if (in_array((($i-1) % $columns)+1, $showColumns) && in_array($i, $showRows)) {
                        ...
                        $itemData = [
                            ...
                            'signalTitleHighlight' => $signalTitleHighlight,
                        ] + $options;
    ...
    {
        //dd($this->_block->pageCustomLang['label']); Esta forma es para pintar el label de los blocks con el lenguaje seleccionado de los filtros.
