...
use Modules\CMS\Helpers\GalleryUploadHandler;
...
class Gallery extends AbstractBlock
{
    ...
    public function editPage()
    {
        ...
        if ($page->exists) {
            ...
            return View::make('admin::cms.pages.gallery', ['paths' => $paths, '_block' => $this->_block, 'can_delete' => Auth::action('gallery.delete', ['page_id' => $page->id]), 'can_edit_caption' => Auth::action('gallery.caption', ['page_id' => $page->id])]);
        } else {
    ...
    public function submitSort()
    {
        ...
        if ($this->_block->getPageId() && !config('admin.config.publishing')) { // update live page if no publishing
