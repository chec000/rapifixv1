<?php namespace Modules\CMS\Libraries\Builder\PageBuilder;

use Illuminate\Support\Facades\Route;
use Illuminate\Support\Facades\Session;
use Modules\CMS\Exceptions\PageBuilderException;
use Modules\CMS\Helpers\Page\PageLoader;
use Modules\CMS\Helpers\Page\Path;
use Modules\CMS\Libraries\Builder\PageBuilder;
use Modules\CMS\Libraries\Builder\ViewClasses\BreadCrumb;
use Modules\CMS\Libraries\Builder\ViewClasses\PageDetails;
use Modules\CMS\Helpers\View\PaginatorRender;
use Modules\CMS\Libraries\Builder\MenuBuilder;
...
use Modules\CMS\Entities\PageSearchData;
...
class DefaultInstance
{
     ...
     * @return int
     */
    public function pageVersion($pageId = 0, $noOverride = false, $version = null)
    {
        ...
        if ($this->previewVersion && $pageId == $this->pageId(true)) {
            ...
            return $version ? $version : PageLang::preload($pageId)->live_version;
        }
    }
     ...
     * @return string
     */
    public function images($fileName, $options = [])
    {
        $imageData = new \stdClass;
        $imageData->file =  $fileName;
        if (empty($options['full_path'])) {
            $imageData->file = '/themes/' . $this->theme . '/images/' . $imageData->file;
        }
        $imageBlock = (new Block);
        $imageBlock->type = 'image';
        return $imageBlock->getTypeObject()->display(serialize($imageData), $options);
...
//                dd($page->pageLang()->url);
    ...
    protected function _block($blockName, $options = [], $fn = 'display')
    {
        ...
        $versionLang = $this->pageVersion($pageId);
        ...
        if (($customBlockData = $this->_getCustomBlockData($blockName)) !== null && !isset($options['page_id'])) {
            ...
            if(Route::current()->getPrefix() != null && Route::current()->getPrefix() == "admin"){
                $globalBlockData = PageBlockDefault::preload($block->id);
            } else {
                $arrGlobalBlockData = PageBlockDefault::where('block_id', '=', $block->id)->where('brand_id', '=', Session::get('portal.main.brand.id'))
                    ->where('country_id', '=', Session::get('portal.main.country_id'))->where('language_id', '=', Session::get('portal.main.language_id'))
                    ->get();
                $globalBlockData = [];
                foreach ($arrGlobalBlockData as $gbd){
                    $globalBlockData[$gbd->language_id] = $gbd;
                }
            }

            $arrPageBlockData = PageBlock::where([
                ['page_id','=', $pageId],
                ['block_id', '=', $block->id],
                ...
                ->get();
            $pageBlockData = [];
            foreach ($arrPageBlockData as $pbd){
                $pageBlockData[$pbd->language_id] = $pbd;
            }
            //$pageBlockData = PageBlock::preloadPageBlock($pageId, $block->id, $this->pageVersion($pageId));

            // get languages
            $loadForLanguages = [!empty($options['language']) ? $options['language'] : \Modules\Admin\Entities\Language::current()];
            if (config('cms.frontend.language_fallback') == 1 && !in_array(config('cms.frontend.language'), $loadForLanguages)) {
                $loadForLanguages[] = config('cms.frontend.language');
            }
            ...
            foreach ($loadForLanguages as $language) {
                if (!empty($pageBlockData[$language])) {
                    ...
                    //print_r($pageBlockData[$language]->version);
                    //$versionLang = $this->pageVersion($pageId, false, $pageBlockData[$language]->version);
                    ...
                    //$versionLang = $this->pageVersion($pageId, false , $globalBlockData[$language]->version );
        ...
        if(empty($options['version'])) {
            $options['version'] = $usingGlobalContent ? 0 : $versionLang;
        }
