...
class DateTimeHelper
{
    ...
    public static function display($dateTime, $format = 'long')
    {
        ...
        return $dateTime->format(config('cms.date.format.'.$format));
    }
    ...
    {
        if ($jqueryDt) {
            if ($date = Carbon::createFromFormat(config('cms.date.format.jq_php'), $jqueryDt)) {
    ...
    public static function mysqlToJQuery($mysqlDt)
    {
        if ($mysqlDt && strtotime($mysqlDt)) {
            return (new Carbon($mysqlDt))->format(config('cms.date.format.jq_php'));
        }
        return '';
    }
