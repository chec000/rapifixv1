@if (isset($shoppingCart) && sizeof($shoppingCart) > 0)
            ...
            <div class="cart-product__content">
                ...
                <div class="cart-product__bottom">
                    <div class="form-group numeric">
                        <input id="input{{ $item['sku'] }}" class="form-control number" maxlength="4"
                               {{ ((isset($item['promo']) && $item['promo']) || (isset($item['is_special']) && $item['is_special'])) ? 'readonly' : '' }}
                        type="numeric" name="qty#{val}" value="{{ $item['quantity'] }}" onchange="changeQtyResumeCart(this)" onkeypress="return esNumero(event)">
                    </div>
    ...
    @endforeach
@else
    <li id="cart-empty" style="text-align: center; margin-top: 50px;" class="cart-empty">@lang('shopping::checkout.quotation.resume_cart.no_items')<br><br>
        <div align="center"><a href="{{ route(\App\Helpers\TranslatableUrlPrefix::getRouteName(session()->get('portal.main.app_locale'), ['products', 'index'])) }}"><button class="button secondary small" type="button">@lang('shopping::checkout.keep_buying')</button></a>
        </div>
    </li>
@endif
