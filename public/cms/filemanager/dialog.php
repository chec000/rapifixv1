...
// If hidden folders are specified
    ...
    \Modules\Admin\Helpers\FileManager::setSecureUpload($subdir);
...
//                console.log(ext_img);
                ...
                dictInvalidFileType: "<?php echo transfm('Error_extension'); ?>",
                        dictFileTooBig: "<?php echo transfm('Error_Upload'); ?>",
                        dictDefaultMessage: "<?php echo transfm('Upload_message'); ?>",
                        dictResponseError: "<?php echo transfm('SERVER ERROR'); ?>",
    ...
    <?php } ?>
                ...
                done("<?php echo transfm('Error_extension'); ?>");
                }
                }
                };
                if (image_editor) {
    ...
    <?php if ($upload_files) { ?>
        ...
        <?php if ($java_upload) { ?>
                            <ul class="nav nav-tabs">
                                <li class="active"><a href="#tab1" data-toggle="tab"><?php echo transfm('Upload_base'); ?></a></li>
                                <li><a href="#tab2" id="uploader-btn" data-toggle="tab"><?php echo transfm('Upload_java'); ?></a></li>
                            </ul>
        ...
        <?php } ?>
                                <form action="dialog.php" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
                                    ...
                                    <div class="fallback">
                                        <h3><?php echo transfm('Upload_file') ?>:</h3><br/>
                                        ...
                                        <input type="submit" name="submit" value="<?php echo transfm('OK') ?>" />
                                    </div>
                                </form>
        ...
        <?php if ($java_upload) { ?>
                                ...
                                <div class="tab-pane" id="tab2">
                                    ...
                                    <div class="upload-help"><?php echo transfm('Upload_java_help'); ?></div>
        <?php } ?>
                            </div>
                        </div>
                    </div>
...
// Should lazy loading be enabled
                ...
                <div class="navbar navbar-fixed-top">
                    <div class="navbar-inner">
                        <div class="container-fluid">
                            ...
                            <div class="brand"><?php echo trans('admin::fileManager.toolbar'); ?></div>
                            <div class="nav-collapse collapse">
                                <div class="filters">
                                    <div class="row-fluid">
                                        <div class="span4 half">
                                            <?php if ($upload_files) { ?>
                                                <button class="tip btn upload-btn" title="<?php echo trans('admin::fileManager.upload_file'); ?>"><i class="rficon-upload"></i></button>
                                            <?php } ?>
                                            <?php if ($create_text_files) { ?>
                                                <button class="tip btn create-file-btn" title="<?php echo transfm(trans('admin::fileManager.new_file')); ?>"><i class="icon-plus"></i><i class="icon-file"></i></button>
                                            <?php } ?>
                                            <?php if ($create_folders) { ?>
                                                <button class="tip btn new-folder" title="<?php echo transfm(trans('admin::fileManager.new_folder')) ?>"><i class="icon-plus"></i><i class="icon-folder-open"></i></button>
                                            <?php } ?>
    <?php if ($copy_cut_files || $copy_cut_dirs) { ?>
                                                <button class="tip btn paste-here-btn" title="<?php echo transfm(trans('admin::fileManager.paste_here')); ?>"><i class="rficon-clipboard-apply"></i></button>
                                                <button class="tip btn clear-clipboard-btn" title="<?php echo transfm(trans('admin::fileManager.clear_clipboard')); ?>"><i class="rficon-clipboard-clear"></i></button>
    <?php } ?>
                                        </div>
        ...
        <?php if (count($ext_music) > 0 or false) { ?>
                                                ...
                                                <?php } ?>
    <?php } ?>
                                            <input accesskey="f" type="text" class="filter-input <?php echo (($_GET['type'] != 1 && $_GET['type'] != 3) ? '' : 'filter-input-notype'); ?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(transfm('Text_filter')); ?>..." value="<?php echo $filter; ?>"/><?php if ($n_files > $file_number_limit_js) { ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>

                                            <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
                                            <label id="ff-item-type-all" title="<?php echo transfm('All'); ?>" <?php if ($_GET['type'] == 1 || $_GET['type'] == 3) { ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-remove icon-white"></i></label>
    ...
    } ?>" href="javascript:void('')" data-sort="name"><?php echo transfm('Filename'); ?></a></li>
                                    <li><a class="sorter sort-date <?php if ($sort_by == "date") {
        echo ($descending) ? "descending" : "ascending";
    } ?>" href="javascript:void('')" data-sort="date"><?php echo transfm('Date'); ?></a></li>
                                    <li><a class="sorter sort-size <?php if ($sort_by == "size") {
        echo ($descending) ? "descending" : "ascending";
    } ?>" href="javascript:void('')" data-sort="size"><?php echo transfm('Size'); ?></a></li>
                                    <li><a class="sorter sort-extension <?php if ($sort_by == "extension") {
        echo ($descending) ? "descending" : "ascending";
    } ?>" href="javascript:void('')" data-sort="extension"><?php echo transfm('Type'); ?></a></li>
                                </ul>
                            </div>
                        </li>
    ...
    <?php if ($show_total_size) { ?>
                            <li><small class="hidden-phone"><span title="<?php echo transfm('total size') . $MaxSizeTotal; ?>"><?php echo transfm('total size') . ": " . makeSize($sizeCurrentFolder) . (($MaxSizeTotal !== false && is_int($MaxSizeTotal)) ? '/' . $MaxSizeTotal . ' ' . transfm('MB') : ''); ?></span></small>
                            </li>
                        <?php } ?>
                    </ul>
                </div>
    ...
    <?php } else { ?>
        ...
        <?php if (isset($folder_message)) { ?>
                        ...
                        echo ($descending) ? "descending" : "ascending";
                    } ?>" href="javascript:void('')" data-sort="name"><?php echo transfm('Filename'); ?></a></div>
                                    <div class="file-date"><a class="sorter sort-date <?php if ($sort_by == "date") {
                            echo ($descending) ? "descending" : "ascending";
                        } ?>" href="javascript:void('')" data-sort="date"><?php echo transfm('Date'); ?></a></div>
                                    <div class="file-size"><a class="sorter sort-size <?php if ($sort_by == "size") {
                            echo ($descending) ? "descending" : "ascending";
                        } ?>" href="javascript:void('')" data-sort="size"><?php echo transfm('Size'); ?></a></div>
                            ...
                            echo ($descending) ? "descending" : "ascending";
                        } ?>" href="javascript:void('')" data-sort="extension"><?php echo transfm('Type'); ?></a></div>
                            ...
                            <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                                ...
                                foreach ($files as $file_array) {
                                    ...
                                    if ($file == '.' || (isset($file_array['extension']) && $file_array['extension'] != transfm('Type_dir')) || ($file == '..' && $subdir == '') || in_array($file, $hidden_folders) || ($filter != '' && $n_files > $file_number_limit_js && $file != ".." && stripos($file, $filter) === false))
                                        continue;
            ...
            <?php if ($file == "..") { ?>
                            ...
                            echo "ellipsis";
                                                ...
                                                <input type="hidden" class="extension" value="<?php echo transfm('Type_dir'); ?>"/>
                                        ...
                                        <?php } ?>
                                                <div class='file-extension'><?php echo transfm('Type_dir'); ?></div>
                                                <figcaption>
                                                    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_folders && !$file_prevent_rename) echo "rename-folder"; ?>" title="<?php echo transfm('Rename') ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>">
                                                        <i class="icon-pencil <?php if (!$rename_folders || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>
                                                    <a href="javascript:void('')" class="tip-left erase-button <?php if ($delete_folders && !$file_prevent_delete) echo "delete-folder"; ?>" title="<?php echo transfm('Erase') ?>" data-confirm="<?php echo transfm('Confirm_Folder_del'); ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" >
                    ...
                    echo $img_width . "x" . $img_height;
                                                ...
                                                <figcaption>
                                                    <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                                        ...
                                                        <a title="<?php echo transfm('Download') ?>" class="tip-right" href="javascript:void('')" onclick="$('#form<?php echo $nu; ?>').submit();"><i class="icon-download"></i></a>
                <?php if ($is_img && $src_thumb != "" && $extension_lower != "tiff" && $extension_lower != "tif") { ?>
                                                            <a class="tip-right preview" title="<?php echo transfm('Preview') ?>" data-url="<?php echo $src; ?>" data-toggle="lightbox" href="#previewLightbox"><i class=" icon-eye-open"></i></a>
                ...
                <?php } elseif ($viewerjs_enabled && in_array($extension_lower, $viewerjs_file_exts)) { ?>
                                                            <a class="tip-right file-preview-btn" title="<?php echo transfm('Preview') ?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
                                                               href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

                <?php } else { ?>
                                                            <a class="preview disabled"><i class="icon-eye-open icon-white"></i></a>
                <?php } ?>
                                                        <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if ($rename_files && !$file_prevent_rename) echo "rename-file"; ?>" title="<?php echo transfm('Rename') ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>">
                                                            <i class="icon-pencil <?php if (!$rename_files || $file_prevent_rename) echo 'icon-white'; ?>"></i></a>

                                                            <a id="delete_file" href="javascript:void('')" class="tip-left erase-button <?php if ($delete_files && !$file_prevent_delete) echo "delete-file"; ?>" title="<?php echo transfm('Erase') ?>" data-confirm="<?php echo transfm('Confirm_del'); ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>">
