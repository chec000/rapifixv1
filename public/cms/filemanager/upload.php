...
{
	response(transfm('forbiden').AddErrorLocation(), 403)->send();
	exit;
}
...
if ( ! empty($_FILES))
{
	$info = pathinfo($_FILES['file']['name']);
                  $info2=$_FILES['file'];
//                  dd($info2);
                    if($info2['type']=='application/vnd.ms-excel'){
	...
	if($extension==='' || $extension=='so' || $extension != $info['extension']){
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		if (!checkresultingsize($_FILES['file']['size'])) {
			response(sprintf(transfm('max_size_reached'),$MaxSizeTotal).AddErrorLocation(), 406)->send();
			exit;
		}
		...
		if ($is_img)
		{
			...
			{
				unlink($targetFile);
				response(transfm("Not enought Memory").AddErrorLocation(), 406)->send();
				exit();
			}
		}
	...
	{
		response(transfm("Error_extension").AddErrorLocation(), 406)->send();
		exit();
	}
}
else // no files to upload
{
	response(transfm("no file").AddErrorLocation(), 405)->send();
	exit();
}
