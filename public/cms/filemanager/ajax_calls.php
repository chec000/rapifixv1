...
{
	response(transfm('forbiden').AddErrorLocation())->send();
	exit;
}
	...
	switch($_GET['action'])
	{
		case 'view':
			if(isset($_GET['type']))
			{
				$_SESSION['RF']["view_type"] = $_GET['type'];
                                                                response(transfm('order_files'))->send();
			}
			else
			{
		...
		case 'save_img':
			...
			if (
				...
				|| ! in_array(strtolower($info['extension']), array( 'jpg', 'jpeg', 'png','svg' ))
			)
			{
				response(transfm('wrong data').AddErrorLocation())->send();
				exit;
			}
			$image_data = get_file_by_url($_POST['url']);
			if ($image_data === false)
			{
				response(transfm('Aviary_No_Save').AddErrorLocation())->send();
				exit;
			}

			if (!checkresultingsize(strlen($image_data))) {
				response(sprintf(transfm('max_size_reached'),$MaxSizeTotal).AddErrorLocation())->send();
				exit;
			}
			...
			if (is_dir($path))
			{
				...
				if ($copy_cut_max_size !== false && is_int($copy_cut_max_size)) {
					if (($copy_cut_max_size * 1024 * 1024) < $sizeFolderToCopy) {
						response(sprintf(transfm('Copy_Cut_Size_Limit'), $msg_sub_action, $copy_cut_max_size).AddErrorLocation())->send();
						exit;
					}
				}
				...
				if ($copy_cut_max_count !== false && is_int($copy_cut_max_count))
				{
					...
					{
						response(sprintf(transfm('Copy_Cut_Count_Limit'), $msg_sub_action, $copy_cut_max_count).AddErrorLocation())->send();
						exit;
					}
				}
