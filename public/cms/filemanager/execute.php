...
if (isset($_GET['action']))
{
	switch($_GET['action'])
	{
		case 'delete_file':
			if ($delete_files){
				...
				if ($fixed_image_creation)
				{
					...
					response(transfm('file_delete'))->send();						
			}
			break;
		case 'delete_folder':
			if ($delete_folders){
		...
		case 'create_file':
			...
			if (strpos($name, '.') === FALSE){
				response(transfm('No_Extension').' '.sprintf(transfm('Valid_Extensions'), implode(', ', $editable_text_file_exts)).AddErrorLocation())->send();
				exit;
			}
			...
			if (!in_array(end($parts), $editable_text_file_exts)) {
				response(transfm('Error_extension').' '.sprintf(transfm('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
				exit;
			}
						...
						foreach($fixed_path_from_filemanager as $k=>$paths)
						{
							...
							if (file_exists($base_dir.$fixed_image_creation_name_to_prepend[$k].$info['filename'].$fixed_image_creation_to_append[$k].".".$info['extension']))
							{
								rename_file($base_dir.$fixed_image_creation_name_to_prepend[$k].$info['filename'].$fixed_image_creation_to_append[$k].".".$info['extension'],$fixed_image_creation_name_to_prepend[$k].$name.$fixed_image_creation_to_append[$k],$transliteration);
                                                                                         ...
                                                                                         response(transfm('rename_file'))->send();
				} else {
					response(transfm('Empty_name').AddErrorLocation())->send();
					exit;
				}
